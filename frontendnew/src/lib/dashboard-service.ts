...
// ✅ pull the real user from your AuthContext
...
/**
 * Map AuthContext user -> Dashboard UserProfile while preserving
 * all the extra dashboard-specific fields already in state (avatar,
 * notification prefs, quiz summaries, etc.).
 */
function mergeAuthIntoProfile(
  ...
  if (!authUser) {
    // Not authenticated: keep previous profile but clear identity fields.
    return {
      ...prev,
      user_id: "guest",
  ...
  return {
    ...prev,
    user_id: authUser.id || prev.user_id || "unknown",
    email: authUser.email ?? prev.email ?? "",
    name: authUser.name ?? prev.name ?? "",
    // keep everything else (profile_picture, preferences, stats, etc.)
  };
}
...
export function useDashboardService() {
  ...
  // ------- initial state (keeps sample data; profile will be overlaid with auth) -------
  ...
  // -------- persist whenever dashboard state changes --------
  useEffect(() => {
    localStorage.setItem("dashboard-state", JSON.stringify(state));
  }, [state]);

  // -------- sync AuthContext user into dashboard profile --------
  useEffect(() => {
    // Wait until auth finishes its initial check; then overlay user into profile.
    if (isLoading) return;
    setState((prev) => ({
      ...prev,
      userProfile: mergeAuthIntoProfile(user, prev.userProfile),
    }));
  }, [user, isLoading]);
  ...
  const actions = useMemo(
    () => ({
      ...
      applyToJob: async (jobId: string) => {
        // Check if resume is uploaded (uses profile_picture sentinel like you had)
        if (state.userProfile.profile_picture === "/professional-headshot.png") {
          const shouldUpload = confirm(
            "You need to upload your resume before applying. Would you like to upload it now?"
          );
          if (shouldUpload) {
        ...
        alert(
          `Application submitted successfully for ${
            state.jobMatches.find((j) => j.job_id === jobId)?.job_title
          }!`
        );
      },
      ...
      saveJob: (jobId: string) => {
        ...
        if (job) {
          alert(
            isSaved
              ? `Job unsaved: ${job.job_title} at ${job.company}`
              : `Job saved: ${job.job_title} at ${job.company}`
          );
        }
      },
      ...
      toggleNotification: (
        type: keyof UserProfile["notification_preferences"]
      ) => {
        setState((prev) => ({
          ...prev,
          userProfile: {
      ...
      resetDashboard: () => {
        setState({
          userProfile: mergeAuthIntoProfile(user, sampleUserProfile), // keep auth identity if available
          skills: sampleSkills,
          jobMatches: sampleJobs,
          mentors: sampleMentors,
          courses: sampleCourses,
    ...
    // Dependencies: when `user` changes we want actions that read it (resetDashboard/applyToJob uses state only)
  ...
  return {
    state,
    actions,
    isAuthenticated, // optional: expose for convenience
  };
}
