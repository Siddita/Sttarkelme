...
const BecomeMentor = () => {
  ...
  const { data: profile, isLoading: profileLoading } = meV1MeGet({
    enabled: isAuthenticated,
    retry: false
  });
  const { data: menteeGoals } = myGoalsV1MeMenteeGoalsGet({ 
    enabled: isAuthenticated && !!profile,
    retry: false
  });
  
  // Get pending goal status requests for mentors
  const { data: pendingGoalRequests, isLoading: pendingRequestsLoading, refetch: refetchPendingRequests, error: pendingRequestsError } = listPendingGoalStatusRequestsV1GoalsStatusRequestsPendingGet({
  ...
  const { data: mentorSkills, isLoading: mentorSkillsLoading, refetch: refetchMentorSkills } = myMentorSkillsV1MeMentorSkillsGet({
    enabled: isAuthenticated && !!profile?.is_mentor,
    retry: false
  });
  
  // Get sessions for mentor approval (role="mentor")
  const { data: mentorSessions, isLoading: sessionsLoading } = listSessionsV1SessionsGet({
    enabled: isAuthenticated && !!profile?.is_mentor,
    role: "mentor",
    retry: false
  });
    ...
    if (profile?.is_mentor && profile?.auth_user_id) {
      const fetchRatings = async () => {
        try {
          ...
          const url = `${API_BASE_URL}/mentorship/v1/mentors/${profile.auth_user_id}/ratings`;
  ...
  const addMentorSkillMutation = addMentorSkillV1MeMentorSkillsPost({
    onSuccess: () => {
  ...
  const deleteMentorSkillMutation = deleteMentorSkillV1MeMentorSkills_SkillId_Delete({
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['my_mentor_skills_me_mentor_skills_get'] });
  ...
  const confirmDeleteSkill = async () => {
    ...
    // Use direct API call with correct URL since the hook has path parameter placeholder issues
    try {
      const url = `${API_BASE_URL}/mentorship/v1/me/mentor/skills/${skillId}`;
      const response = await fetch(url, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token.trim()}`,
      ...
      queryClient.invalidateQueries({ queryKey: ['my_mentor_skills_v1_me_mentor_skills_get'] });
