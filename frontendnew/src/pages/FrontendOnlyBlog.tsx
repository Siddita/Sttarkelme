...
// Direct news fetching without backend
...
const FrontendOnlyBlog = () => {
  ...
  const fetchRedditNews = async (limit: number = 10): Promise<DirectNewsItem[]> => {
    try {
  ...
  const fetchNews = async () => {
    ...
    try {
      ...
      if (selectedSource === 'all') {
        ...
        const [redditArticles, hnArticles, devtoArticles] = await Promise.all([
          fetchRedditNews(5),
          fetchHackerNews(5),
          fetchDevToNews(5)
        ]);

        allArticles = [...redditArticles, ...hnArticles, ...devtoArticles];
      } else if (selectedSource === 'reddit') {
        allArticles = await fetchRedditNews(15);
      } else if (selectedSource === 'hackernews') {
        allArticles = await fetchHackerNews(15);
      } else if (selectedSource === 'devto') {
        allArticles = await fetchDevToNews(15);
      }
  ...
  const searchNews = async (query: string) => {
    ...
    try {
      // For search, we'll use Reddit's search functionality
      const response = await fetch(`https://www.reddit.com/r/programming/search.json?q=${encodeURIComponent(query)}&limit=20&sort=new`);
      ...
      const data = await response.json();
      const searchResults = data.data?.children?.map((post: any) => ({
        id: `search-${post.data.id}`,
        title: post.data.title,
        description: post.data.selftext || post.data.title,
        url: `https://reddit.com${post.data.permalink}`,
        publishedAt: new Date(post.data.created_utc * 1000).toISOString(),
  ...
  // News card component
  const NewsCard = ({ article }: { article: DirectNewsItem }) => (
    <motion.div
              ...
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-12">
                  ...
                  <h1 className="text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-normal mb-6 leading-tight animate-fade-in text-[#2D3253]">
                    Direct <span className="bg-gradient-primary bg-clip-text text-transparent">News</span>
                  </h1>
            ...
            <div className="px-4 sm:px-6 lg:px-8 pb-20">
              {error && (
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-semibold text-red-600 mb-2">Error Loading News</h3>
              ...
              {!isLoading && !error && articles.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-muted-foreground">
                    {isSearching ? 'Try adjusting your search terms.' : 'No articles available from the selected source.'}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="px-4 sm:px-6 lg:px-8 pb-20">
              <div className="max-w-2xl mx-auto">
                <Card className="p-8 text-center bg-card/50 backdrop-blur-sm border-primary/20">
                  ...
                  <h3 className="text-2xl font-bold mb-2">Frontend-Only News</h3>
                  <p className="text-muted-foreground mb-6">
                    This news service runs entirely in your browser, fetching data directly from external APIs 
                    without requiring any backend services.
                  </p>
                  <div className="text-sm text-muted-foreground">
                    <p>Sources: Reddit, Hacker News, Dev.to</p>
                    <p>No API keys required • No backend needed • Real-time updates</p>
                  </div>
                </Card>
              </div>
            </div>
...
export default FrontendOnlyBlog;
