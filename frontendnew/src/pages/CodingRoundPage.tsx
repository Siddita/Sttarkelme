...
export default function CodingRoundPage() {
  ...
  const generateCodingChallengeMutation = generateQuestionCodingGenerateQuestion_Post({
    onSuccess: (data) => {
  ...
  const { data: codingHealthStatus, isLoading: isHealthCheckLoading, refetch: checkCodingHealth } = codingHealthCheck_Get({
    enabled: false, // Only check when explicitly called
    retry: 1
  });
